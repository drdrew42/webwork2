...
use WeBWorK::Utils qw/ decodeAnswers newEmailAPI /;
...
#           createEmailSenderTransportSMTP is defined in ContentGenerator
		...
		my $email = {
			to => join(",", @recipients),
			from => 'rationarium.org <system@webwork.rationarium.org>',
			"h:reply-to" => $sender,
			subject => $subject,
		...
		## extra headers
		$email->{'h:X-Remote-Host'} = $remote_host;
		$email->{'h:X-WeBWorK-Module'} = $module if defined $module;
		$email->{'h:X-WeBWorK-Course'} = $courseID if defined $courseID;
		if ($user) {
			$email->{'h:X-WeBWorK-User'} = $user->user_id;
			$email->{'h:X-WeBWorK-Section:'} = $user->section;
			$email->{'h:X-WeBWorK-Recitation:'} = $user->recitation;
		}
		$email->{'h:X-WeBWorK-Set:'} = $set->set_id if $set;
		$email->{'h:X-WeBWorK-Problem:'} = $problem->problem_id if $problem;
